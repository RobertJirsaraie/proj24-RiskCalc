...
if TARG == "TARGET_bpm_totalprob_resid" and PREFIX == "Preproc6_Future":
    X = df[['TARGET_bpm_totalprob_r']]
    y = df['TARGET_bpm_totalprob_future']
    model = LinearRegression(); model.fit(X, y)
    df['TARGET_bpm_totalprob_resid'] = y - model.predict(X)
...
ERRORS.to_csv(f'{ANALYSIS}/df-{SET}_mod-XGB_accuracy.csv',index=False)
RESULTS.to_csv(f'{ANALYSIS}/df-{SET}_mod-XGB_predict.csv',index=False)
IMPORTANCE.to_csv(f'{ANALYSIS}/df-{SET}_mod-XGB_impurity.csv',index=True)
HYPERPARAMETERS.to_csv(f'{ANALYSIS}/df-{SET}_mod-XGB_parameters.csv',index=True)
