...
for (FILE in grep("Preproc*",list.files(ROOT, pattern="sex.*accuracy", recursive=TRUE, full.names=TRUE),value=TRUE)){
    DF<-read.csv(FILE); print(paste0("Loaded File: ", FILE))
    SEX<-gsub("sex-", "", unlist(strsplit(basename(FILE),"_"))[2]) 
    FEATURES<-gsub("df-","",unlist(strsplit(basename(FILE),"_"))[1])
    DF[,'featset']<-FEATURES; DF[,'subgroup']<-SEX
...
JITTER<-ddply(ACCURACY, .(result, subgroup, featset, split), summarize,
            test_cod = mean(test_cod, na.rm = TRUE),
            result = unique(result)
)

SUMMARY <- ddply(ACCURACY, .(result, featset, subgroup), summarize,
...
SUMMARY<-SUMMARY[order(SUMMARY$test_cod,decreasing=TRUE), c("result","featset","test_cod","subgroup")]
...
FS4<-ggplot() + 
    ...
    geom_jitter(JITTER, width=0.25, size=2, alpha=0.65, mapping=aes(x=featset, y=test_cod, colour=subgroup)) + 
    geom_point(SUMMARY, mapping=aes(x=featset, y=test_cod, colour=subgroup, group=subgroup), size=11, shape=18, color="white") + 
    geom_point(SUMMARY, mapping=aes(x=featset, y=test_cod, colour=subgroup, group=subgroup), size=9, shape=18, alpha=0.65) + 
    scale_color_manual(values=c("#b46be8","#6baae8")) +
...
ggsave(plot=FS4,filename=paste0(ROOT,"/Visualize/FS4_SexDiff_",{TODAY},".png"),device="png",width=15, height=8.5,units='in')
